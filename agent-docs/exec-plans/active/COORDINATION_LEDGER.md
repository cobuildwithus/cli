...
| codex-gpt5-builder-codes-2026-03-04 | Append Base ERC-8021 builder suffix in CLI local transaction execution paths via shared wire helper | `src/wallet/local-exec.ts`, `tests/wallet-local-exec.test.ts`, `agent-docs/exec-plans/active/COORDINATION_LEDGER.md` | add wire-backed builder suffix application helper usage in local tx sending path | Keep idempotency behavior unchanged; only mutate outbound calldata before broadcast | 2026-03-04 |
